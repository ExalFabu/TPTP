...
const parseRow = (
  ...
  if (rowValues[0] === 'Periodo') return { toSkip: true };
  if (rowValues[3] === undefined) {
    return { isOppureRow: true };
  }
  const languageUrl: string =
    $(elem).children('* img').attr('src') ?? ('' as string);
  const lectureName =
    $(elem).children('* a').first().text() || rowValues[3].split('\n')[0];
  ...
  lecture.language =
    languageUrl.match(/en\.png$/) !== null
      ? PolitoLanguage.l_en
      : PolitoLanguage.l_it;
  lecture.isMandatory = isMandatory;
  lecture.alternative = undefined;
...
const parseTable = (
  ...
  const filteredLectures = lectures.filter(_ => true) as LectureDraft[];
  lectures.forEach((lecture, index) => {
    if ('isOppureRow' in lecture) {
      (filteredLectures[index - 1] as PolitoLecture).alternative = (
        filteredLectures[index + 1] as PolitoLecture
      ).codice;
      (filteredLectures[index + 1] as PolitoLecture).alternative = (
        filteredLectures[index - 1] as PolitoLecture
      ).codice;
...
const fetchUrl = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!isValidUrl(urlToParse)) {
    return res.status(400).json({ error: 'url di partenza non valido' });
  }
  ...
  const realUrlToParse = $("[aria-label='Piano di studi completo']")
    .attr('href')
    ?.trim();
  if (!realUrlToParse || !isValidUrl(realUrlToParse)) {
    return res
      .status(400)
      .json({ error: `url non valido (${realUrlToParse})` });
  }
  let tries = 10;
  let realHtml;
  while (tries > 0 && !realHtml) {
    realHtml = await fetch(
      changeYear(realUrlToParse, parseInt(req.query.year as string) + 1),
  ...
  console.log("Fetching required " + (10 - tries) + " tries");
  if (!realHtml) {
    return res
      .status(400)
      .json({ error: 'risposta ricevuta da PoliTo non valida' });
  }
  ...
  const apiResponse: FetchFromPolitoResponse =
    pathways.length > 0 ? { pathways } : { error: 'nessun percorso trovato' };

  return res
    .status(200)
    .setHeader('Cache-Control', `max-age=0, s-maxage=${60 * 60 * 24 * 2}`)
    .json(apiResponse);
};
